...
**Connector hygiene:**
- Prefer orthogonal connectors over direct diagonals for process and architecture diagrams.
- Add `<Array as="points">` waypoints for long edges, cross-lane edges, and multi-branch decision fan-outs.
- Do a final pass specifically for line cleanliness: reroute any edge that crosses through unrelated nodes when a cleaner path is available.
- If the user provides screenshots or reports an ugly overlap, treat that as a geometry bug and update the XML routing before changing colors or labels.
- For flowcharts, prefer branch “gutters” and shared buses over letting outputs cut through process boxes.
- For linear left-to-right flows, align the participating nodes to a shared baseline so the primary connector can stay straight.
- If a container boundary is what creates the elbow, adjust geometry first; do not keep a needless bend just to preserve the old box placement.
- In existing diagrams, do a final normalization pass on comparable boxes so rows and columns share consistent heights, widths, and spacing where feasible.
- When improving an existing diagram, preserve the overall layout intent and polish geometry in place instead of making a dramatic rearrangement unless the user asks for a redesign.
- Consider any connector crossing a node body or text block a blocking defect.
